...
class measure():
    ...
    def __init__(self, filepath1, filename1):
        ...
        self.log_switch=threading.Event()
...
#            self.temp_ru=ls_th.get_temp(ru_channel)
...
#            self.temp_cx=ls_th.get_temp(cx_channel)
...
#            self.htr_pw=ls_th.get_heater(cx_channel)
...
#               self.htr_pw_hist=np.delete(self.htr_pw_hist,0)
            ...
            self.hps_mag=hps.get_mag_field()
            self.hps_mag_field_hist=np.append(self.hps_mag_field_hist,self.hps_mag_field)
            if len(self.temp_ru_hist)>len_hist:
                self.hps_mag_field_hist=np.delete(self.hps_mag_field_hist,0)

            self.hps_curr=hps.get_ps_field()
            self.hps_curr_field_hist=np.append(self.hps_curr_field_hist,self.hps_curr_field)
            if len(self.temp_ru_hist)>len_hist:
                self.hps_curr_field_hist=np.delete(self.hps_curr_field_hist,0)

            self.hps_pshtr=hps.get_switch_htr()

            self.vps_mag=vps.get_mag_field()
            self.vps_mag_field_hist=np.append(self.vps_mag_field_hist,self.vps_mag_field)
            if len(self.temp_ru_hist)>len_hist:
                self.vps_mag_field_hist=np.delete(self.vps_mag_field_hist,0)

            self.vps_curr=vps.get_ps_field()
            self.vps_curr_field_hist=np.append(self.vps_curr_field_hist,self.vps_curr_field)
            if len(self.temp_ru_hist)>len_hist:
                self.vps_curr_field_hist=np.delete(self.vps_curr_field_hist,0)

            self.vps_pshtr=vps.get_switch_htr()

            self.angle=rot.get_angle()
    ...
    def sweep_field(start, stop, rate, phi, theta):

        dummy=rot.rotate(phi)
        dummy2=True
        while dummy2:
            dummy2=False
            if rot.get_status==True:
                dummy2=True
            time.sleep(2)
        ...
        dummy=hps.sweep_field(hps_start,0.4,driven=True)
        dummy=vps.sweep_field(vps_start,0.4,driven=True)
